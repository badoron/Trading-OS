...
const TOS_IBKR_FLEX = {
  ...
  // Four waits between five attempts: 15s + 30s + 45s + 60s = 150s.
  // Keeps the total retry wait below the three-minute operational limit.
  RETRY_DELAYS_MS: [15000, 30000, 45000, 60000],
  ...
  sendRequest() {
    for (
      ...
      if (attempt < this.MAX_RETRIES) {
        const waitMs =
          this.RETRY_DELAYS_MS[attempt - 1] || 0;

        Logger.log(
          'IBKR 1001 received. Waiting ' +
    ...
    throw new Error(
      'IBKR temporarily could not generate the Flex statement. ' +
      'ErrorCode=1001 | Attempts=' +
      this.MAX_RETRIES +
      ' | Existing cached XML was not changed. ' +
      'Please retry Full Synchronization later.'
    );
  },
