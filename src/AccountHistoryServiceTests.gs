...
function testAccountHistoryServiceRejectsIncompatibleSchema() {
  const spreadsheet =
    createAccountHistorySpreadsheetMock_();
  ...
  assertAccountHistoryServiceEqual_(
    'incompatible schema throws',
    true,
    error instanceof Error
  );

  assertAccountHistoryServiceEqual_(
    'schema mismatch error is clear',
    true,
    String(error.message).indexOf(
      'schema mismatch'
    ) !== -1
  ...
  assertAccountHistoryServiceDeepEqual_(
    'no snapshot written on mismatch',
    before,
    sheet.getData()
  );

  Logger.log(
    'PASS - testAccountHistoryServiceRejectsIncompatibleSchema'
  );
