...
const TOS_DASHBOARD_BUILDER = {
   ...
   *
   * RealizedPnL is summed exactly as stored in MASTER_TRADES.
   * It is not adjusted for Commission again.
   *
   * @param {Object[]} trades Normalized MASTER_TRADES records.
   * @return {Object} Trade summary.
   */
  summarizeTrades_(trades) {
    const result = {
      total: 0,
      open: 0,
      partialExit: 0,
    ...
    (trades || []).forEach(trade => {
      ...
      result.total++;
      result.realizedPnL += this.number_(
    ...
    result.realizedPnL = this.roundMoney_(
      result.realizedPnL
    );
