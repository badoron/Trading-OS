 ...
 * MVP: APPROVE rows from IMPORT_REVIEW into MASTER_TRADES + TRADE_LEGS.
 */
...
const TOS_IMPORT_APPROVER = {
  ...
  approvePendingImports() {
    ...
    const reviewSheet = ss.getSheetByName(this.IMPORT_REVIEW);
    const masterSheet = ss.getSheetByName(this.MASTER_TRADES);
    const legsSheet = ss.getSheetByName(this.TRADE_LEGS);

    if (!reviewSheet) throw new Error('Missing sheet: ' + this.IMPORT_REVIEW);
    ...
    review.rows.forEach(item => {
      ...
      if (!group) {
        this.setCell_(reviewSheet, rowNumber, review.headers, 'ReviewStatus', 'ERROR');
        this.setCell_(reviewSheet, rowNumber, review.headers, 'DecisionReason', 'Approved group was not found in current open positions.');
        skipped++;
        return;
      }

      const tradeId = this.buildTradeId_(group.groupId);

      if (existingTradeIds[tradeId]) {
        this.updateReviewImported_(reviewSheet, rowNumber, review.headers, tradeId, 'Already imported.');
        skipped++;
        return;
      }

      this.appendMasterTrade_(masterSheet, master.headers, tradeId, group);
      this.appendTradeLegs_(legsSheet, legsTable.headers, tradeId, group);

      this.updateReviewImported_(reviewSheet, rowNumber, review.headers, tradeId, 'Imported to MASTER_TRADES and TRADE_LEGS.');

      existingTradeIds[tradeId] = true;
      imported++;
    });

    Logger.log('Import approval completed. Imported=' + imported + ', Skipped=' + skipped);
  ...
  appendMasterTrade_(sheet, headers, tradeId, group) {
    ...
    const values = {
      ...
      BrokerPositionID: group.sourcePositionIds.join(','),
      StrategyVersion: 'DDC-MVP-1',
      EntryDecisionID: group.groupId,
      RealizedPnL: '',
      Tags: 'IBKR,DDC,MVP',
      EntryThesis: 'Imported from active IBKR open positions.',
      ExitReason: ''
    };

    this.appendMappedRow_(sheet, headers, values);
  },

  appendTradeLegs_(sheet, headers, tradeId, group) {
    group.legs.forEach((leg, index) => {
      const values = {
        LegID: tradeId + '-L' + (index + 1),
        TradeID: tradeId,
        StrategyID: 'DDC',
        Symbol: leg.underlyingSymbol || group.symbol,
        BrokerContractID: leg.conid || '',
        Expiration: leg.expiry || '',
        CallPut: leg.putCall || '',
        LongShort: leg.side || '',
        Quantity: leg.position || '',
        Strike: leg.strike || '',
        EntryPrice: leg.costBasisPrice || leg.openPrice || '',
        ExitPrice: '',
        CurrentPrice: leg.markPrice || '',
        MarketValue: leg.positionValue || '',
        UnrealizedPnL: leg.fifoPnlUnrealized || '',
        Source: 'IBKR_OPEN_POSITION'
      };
  ...
  updateReviewImported_(sheet, rowNumber, headers, tradeId, reason) {
    this.setCell_(sheet, rowNumber, headers, 'CreateOrLinkTradeID', tradeId);
    this.setCell_(sheet, rowNumber, headers, 'ImportDecision', 'IMPORTED');
    this.setCell_(sheet, rowNumber, headers, 'ReviewStatus', 'IMPORTED');
    this.setCell_(sheet, rowNumber, headers, 'DecisionReason', reason);
    this.setCell_(sheet, rowNumber, headers, 'ReviewedAt', new Date());
  },

  appendMappedRow_(sheet, headers, values) {
  ...
  getExistingIds_(sheet, table, idColumn) {
    ...
    if (colIndex < 0) return map;

    table.rows.forEach(item => {
      const id = String(item.row[colIndex] || '').trim();
      if (id) map[id] = true;
    });
  ...
  buildTradeId_(groupId) {
    ...
    return 'TRD-' + hex.substring(0, 12).toUpperCase();
  }
};

function testApproveImports() {
  return TOS_IMPORT_APPROVER.approvePendingImports();
}