...
## v3.1.0-rc.2 — Full Synchronization and residual lifecycle

### Added
- Added one-click `Full Synchronization` as the primary Trading OS workflow.
- Added stage-aware orchestration for XML download, parsing, DDC detection, Import Review approvals, trade lifecycle, residual synchronization, Dashboard, and HOME.
- Added a unique Run ID and operational summary for each complete synchronization.
- Added synchronization of OPEN residual positions against the current IBKR Open Positions snapshot.

### Changed
- Limited IBKR Flex retries to five attempts with bounded delays totaling 150 seconds.
- Dashboard and HOME now refresh after residual lifecycle synchronization.
- Promoted the runtime version to `v3.1.0-rc.2`.

### Fixed
- Cached XML is preserved when IBKR returns temporary generation error `1001`.
- Residual positions no longer remain permanently OPEN after disappearing from the broker snapshot.

### Validated
- Completed a live end-to-end Full Synchronization against IBKR.
- Verified three stale residuals closed and one current residual remained open.
